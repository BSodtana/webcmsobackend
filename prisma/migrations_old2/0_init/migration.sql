...
CREATE TABLE `organizations` (
    ...
    `orgType` ENUM('CLUB', 'DIVISION', 'OTHER') NOT NULL DEFAULT 'DIVISION',
...
CREATE TABLE `projects` (
    ...
    `academicYear` INTEGER NOT NULL DEFAULT 202401,
...
CREATE TABLE `projectdata` (
    ...
    `professorFullName` VARCHAR(100) NULL DEFAULT '',
    `professorAffiliation` VARCHAR(100) NULL DEFAULT '',
...
CREATE TABLE `projectparticipantrecruit` (
    ...
    `yearAllowed` VARCHAR(191) NOT NULL DEFAULT '1,2,3,4,5,6',
...
CREATE TABLE `userslogout` (
    `killedTokenAutoID` INTEGER NOT NULL AUTO_INCREMENT,
    `uuid` VARCHAR(191) NOT NULL,
    `token` VARCHAR(10000) NULL,
    `killedDateTime` DATETIME(0) NULL DEFAULT CURRENT_TIMESTAMP(0),

    UNIQUE INDEX `killedTokenAutoID`(`killedTokenAutoID`),
    INDEX `uuid`(`uuid`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- AddForeignKey
ALTER TABLE `organizations` ADD CONSTRAINT `organizations-parentOrg` FOREIGN KEY (`parentOrg`) REFERENCES `organizations`(`orgID`) ON DELETE CASCADE ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `users` ADD CONSTRAINT `usercredentials-user-studentID` FOREIGN KEY (`studentID`) REFERENCES `usercredentials`(`studentID`) ON DELETE CASCADE ON UPDATE CASCADE;
