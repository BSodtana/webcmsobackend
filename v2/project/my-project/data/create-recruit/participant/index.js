...
router.post('/:projectID', async (req, res) => {
  ...
  const { registerFrom, registerUntil, maxNumber, yearAllowed } = req.body
  if (!token) {
    res.status(401).json({
      status: 'unauthorized',
  ...
  if (token) {
    try {
      ...
      if (projectData.studentID === tokenData.data.studentID) {
        const currentRows = await prisma.projectparticipantrecruit.count({
          where: { projectID: { equals: projectID } },
        })
        console.log(currentRows)
        const recruitID = `P${currentRows + 1}-${projectID}`
        await prisma.projectparticipantrecruit.create({
          data: {
...
router.put('/:recruitID', async (req, res) => {
  ...
  const { registerFrom, registerUntil, maxNumber, yearAllowed, isAllowed } =
    req.body
  if (!token) {
    res.status(401).json({
      status: 'unauthorized',
  ...
  if (token) {
    try {
      const tokenData = await VerifyUserJWT(token)
      await prisma.projectparticipantrecruit.update({
        where: { participantRecruitID: recruitID },
        data: {
          participantRecruitID: recruitID,
          registerFrom: registerFrom,
          registerUntil: registerUntil,
