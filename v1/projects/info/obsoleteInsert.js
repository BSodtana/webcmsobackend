...
async function obsoleteDataFetch (req, res) {
  ...
  if (jwt) {
    ...
    if (token.isAuthenticated) {
      const data = await db.query('SELECT * FROM projects_data WHERE project_id = ?', [projectId])
      if (data.length) {
        const info = data[0]
        const constructor = {
...
function sumObject (obj) {
  ...
  let sum = 0
  object.map((data) => {
    sum += parseInt(data.unit.match(/\d/g).join(''), 10) * data.unitPrice
    return sum
  })
...
async function getOwnerOrgName (owner_org) {
  const owner_type = await db.query('SELECT type FROM clubs WHERE id = ?', [owner_org])
  let text
  if (owner_type[0].type === 'CLUB') {
    const data = await db.query('SELECT clubs.name AS clubName, divisions.name AS divName FROM clubs JOIN divisions ON clubs.owner_division = divisions.id WHERE clubs.id = ?', [owner_org])
  ...
  if (owner_type[0].type === 'DIVISION') {
    const data = await db.query('SELECT name FROM clubs WHERE id = ?', [owner_org])
...
async function getStudentData (stu_id) {
  let text
  const data = await db.query('SELECT title, first_name, middle_name, last_name, phone_number FROM users WHERE student_id = ?', [stu_id])
  const t = data[0]
...
module.exports = obsoleteDataFetch
