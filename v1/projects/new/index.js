...
const InsertNewProject = require('./insertNewProject')
...
router.post('/', async (req, res) => {
  InsertNewProject(req, res)
})

router.post('/obsolete', async (req, res) => {
  const { project_name, place_in_cmu, place_outside_cmu, date_prepare_start, date_prepare_end, date_event_start, date_event_end, date_summation_start, date_summation_end, owner_org, owner_student, prof, prof_aff, other_org, cmso_proj_type, cmu_proj_type, cmu_med_grad, cmso_mission_type, cmu_med_org, background, aims, participant_student, participant_prof, participant_student_staff, participant_fac_staff, participant_outside, goal_qualitative, indicator_participant, indicator_satisfactory, indicator_aims, output, plan, stageDo, check, act, fund_fac, fund_extracir, fund_sport, fund_other, fund_name } = req.body.constructor
  try {
    // TRY INITIATE ID PROJECTS
    const countProj = await db.query('SELECT COUNT(owner_org) AS count FROM projects_data WHERE owner_org = ?', [owner_org])
    const orgCount = parseInt(countProj[0].count)
    const currentDTM = new Date()
    const month = currentDTM.getUTCMonth()
    const year = currentDTM.getUTCFullYear()
    const monthText = getMonthText(month)
    const proj_id = `${owner_org}${orgCount + 1}-${year}${monthText})`
    console.log(req.body.constructor)
    const query = await db.query('INSERT INTO projects_data (project_id, project_name, place_in_cmu, place_outside_cmu,date_prepare_start,date_prepare_end,date_event_start,date_event_end,date_summation_start,date_summation_end,owner_org,owner_student,prof,prof_aff,other_org,cmso_proj_type,cmu_proj_type,cmu_med_grad,cmso_mission_type,cmu_med_org,background,aims,participant_student,participant_prof,participant_student_staff,participant_fac_staff,participant_outside,goal_qualitative,indicator_participant,indicator_satisfactory,indicator_aims,output,plan,stageDo,stageCheck,act,fund_fac,fund_extracir,fund_sport,fund_other, fund_name) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [proj_id, project_name, place_in_cmu, place_outside_cmu, date_prepare_start, date_prepare_end, date_event_start, date_event_end, date_summation_start, date_summation_end, owner_org, owner_student, prof, prof_aff, other_org, cmso_proj_type, cmu_proj_type, cmu_med_grad, cmso_mission_type, cmu_med_org, background, aims, participant_student, participant_prof, participant_student_staff, participant_fac_staff, participant_outside, goal_qualitative, indicator_participant, indicator_satisfactory, indicator_aims, output, plan, stageDo, check, act, fund_fac, fund_extracir, fund_sport, fund_other, fund_name])
    const query2 = await db.query('INSERT INTO projects (project_id, owner_id, division_id, allow_register) VALUES (?,?,?,?)', [proj_id, owner_student, owner_org, 0])
